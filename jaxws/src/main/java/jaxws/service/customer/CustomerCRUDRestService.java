...
public class CustomerCRUDRestService {
	...
	@GET
	// @Path("search")
	public Customer getCustomerByFirstnameName(
			@MatrixParam("firstname") String firstname,
			@MatrixParam("surname") String surname) {
		System.out.println("getCustomerByFirstnameName : " + firstname + " - "
				+ surname);
		return new Customer("John", "Smith", "jsmith@gmail.com", "1234565");
	}
	...
	@Path("userAgentRep")
	@Produces(MediaType.TEXT_PLAIN)
	public Response echoUserAgentWithReponse(
			@HeaderParam(value = "User-Agent") String userAgent) {
		System.out.println("echoUserAgentWithReponse : " + userAgent);
		return Response
				.ok(userAgent + " from the server", MediaType.TEXT_PLAIN)
				.build();
	}
}
